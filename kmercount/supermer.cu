...
__global__ void cuda_build_supermer(char *seq, char *kmers, int klen, int mlen, unsigned int seq_len,
    ...
    int per_block_seq_len, int window){
    ...
    if(i <  nKmer) {
        ...
        for(int i = st_char_th; i < end_char_th && (i ) < nKmer ; i++) {
            ...
            if(validKmer)
            {  
                //find_minimizer(cur_kmer, order, klen, mlen);
    ...
    keyType* h_send_smers, unsigned char* h_send_slens, int n_kmers, int rank )
{
    ...
    // int tmp_seq_len = ((seq_len+window-1/window));
    //  g = (tmp_seq_len + (b -1) ) / (b);
    // int per_block_seq_len = (tmp_seq_len  + (g -1) ) / g;
    // cout << seq_len << "  info " << g << " " << per_block_seq_len << endl;

    // Kernel call
    cuda_build_supermer<<<g, b>>>(d_seq, d_kmers, klen, mlen, seq_len, d_supermers, d_slen, d_owner_counter, nproc, p_buff_len, per_block_seq_len, window);
...
__global__ void cu_kcounter_smer(KeyValue* hashtable, const keyType* kvs, const unsigned char* slens,  unsigned int numkvs, int klen, keyType mask)
{
    ...
    if (threadid < numkvs){
        ...
        {
            // uint64_t cur_ckmer =  (cur_csmer >> (2*(31-(klen+k -1)))) & mask; 
