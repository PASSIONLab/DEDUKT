...
/*
 * Trims any info. following the name, and the newline character.
 */
inline static char* get_trimmed_fq_name(char* header) {
	assert(header != NULL);
	assert(header[0] == '@');
	// truncate at the first space or newline character
	size_t origlen = strlen(header);
	for (int i = 0; i < origlen; i++) {
		if (isspace(header[i]) || header[i] == '\n') {
			header[i] = '\0';
			break;
		}
	}
	assert(header[strlen(header)-1] != '\n');
	assert(!isspace(header[strlen(header)-1]));
	assert(strlen(header) <= origlen);
	return header;
}
...
static int64_t get_fptr_for_next_record(fq_reader_t fqr, int64_t offset)
{
...
#ifndef NO_GZIP
    if(fqr->gz || !fqr->f) DIE("Can not fseek in a compressed file! (%s)\n", fqr->name);
#else
    if(!fqr->f) DIE("Must fseek in an open file! (%s)\n", fqr->name);
#endif
    char lastName[256];
    lastName[0] = '\0';
    ...
    LOGF("Finding fptr for next record of %s after %lld\n", fqr->name, (lld) offset);
    ...
    while (1) {
        ...
        DBG("Read line of %lld length at offset %lld count %d: %s", (lld) getLengthBuffer(fqr->buf), (lld) prev_offset, count, buf);
        if (buf[0] == '@') {
            ...
            if (possibleHeaderLine > 0 && count == possibleHeaderLine + 1) {
               ...
               if (possibleHeaderLine > 0) DIE("This should not happen!  possibleHeaderLine=%d at %lld.  Read from %lld offset: '%s'\n", possibleHeaderLine, (lld) new_offset, (lld) offset, getStartBuffer(readLines));
...
void open_fq(fq_reader_t fqr, const char *fname, int cached_io, const char *base_dir) {
    ...
    if (cached_io) {
...
#ifndef NO_GZIP
        sprintf(fqr->name, "%s/%s%s", base_dir, get_basename(bname, fname), GZIP_EXT);
        ...
        if (fread(&(fqr->size), sizeof(int64_t), 1, f) != 1) DIE("Invalid uncompressedSize file: %s\n", fname2);
        fclose_track(f);
        fqr->start_read = 0;
        ...
        return;
#else
        sprintf(fqr->name, "%s/%s", base_dir, get_basename(bname, fname));
        get_rank_path(fqr->name, MYTHREAD);
    ...
    if (setvbuf(fqr->f, NULL, _IOFBF, (size_t) FQ_READER_BUFFER_SIZE) != 0)
        WARN("Could not setvbuf on %s to %d! %s", fname, (int) FQ_READER_BUFFER_SIZE, strerror(errno));
...
{
    open_fq(fqr, fname, 0, NULL); // never cached_io so the file is read in partitions
    assert(fqr->f);
...
#ifndef NO_GZIP
    sprintf(my_fname, "%s" GZIP_EXT, my_fname);
    ...
    do {
        ...
        while (bytesWritten < bytesRead) {
           ...
           if (!thisWrite) DIE("Could not write %lld bytes to %s\n", (lld) bytesRead - bytesWritten, my_fname);
    ...
    if (fd != -1) {
        if (ftruncate(fd, my_fsize) == -1) {
            ...
            DBG("Opened shm file %s for writing of size %lld\n", my_fname, (lld) my_fsize);
            ...
            if (tmpaddr == MAP_FAILED) {
                ...
                if (bytes_read != my_fsize) {
                    WARN("Read the wrong number of bytes: %lld vs %ld for %s\n", 
        ...
        if (close(fd) != 0)
            WARN("Could not close the shm file %s! %s", my_fname, strerror(errno));
        else
            DBG("Wrote %s of %lld size.\n", my_fname, (lld) my_fsize);
    } else {
        err = errno;
    }
#endif // NO_GZIP
...
void hexifyId(char *name, uint8_t libnum, uint64_t *id1, uint64_t *id2, uint64_t step) {
    if (name[0] != '@') DIE("Could not hexify fastq read header '%s' (%lld, %lld step %lld)\n", name, (lld) *id1, (lld) *id2, (lld) step);
    int len = strlen(name);
    int pairlen = len;
    int pair = 0;
    uint64_t *id = id1;
    ...
    char newname[32];
    sprintf(newname, "%02x%llx%s", libnum, (unsigned long long int) *id, (pairlen==len?"":name + pairlen));
    if (strlen(newname) >= MAX_READ_NAME_LEN) DIE("Please increase MAX_READ_NAME_LEN as %s it too large now (from %s)\n", newname, name);
...
int get_next_fq_record(fq_reader_t fqr, Buffer id, Buffer nts, Buffer quals) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 0) {
            ...
            char* read_name = get_trimmed_fq_name(buf); // new function for long read names - grabs everything before the ' RQ=??'
            if (!read_name)
...
void close_fq(fq_reader_t fqr)
{
    ...
    if (fqr->f) {
        ...
        if (frac_ambig > 0.01)
            serial_printf("Thread %d found %ld ambiguous characters (out of %ld)\n", 
                          MYTHREAD, num_ambig, num_chars_read);
    }
}
...
int64_t estimate_fq(char *fname, int sampledReads, int64_t *estimatedReads, int64_t *estimatedBases)
{
    ...
    for(i = 0; i < max; i++) {
         ...
         if (buf[0] != '@') {
             ...
             DIE("Improper fastq file (%s:%lld) - First line does not start with '@': %s", fname, fpos, buf);
         }
         if (!get_next_line(fqr)) {
             long long fpos = fqr->fpos;
             DIE("Improper fastq file (%s:%lld) - no second line after %s", fname, fpos, buf);
         }
         int seqLen = getLengthBuffer(fqr->buf);
         bases += seqLen - 1;
         if (!get_next_line(fqr)) {
             long long fpos = fqr->fpos;
             DIE("Improper fastq file (%s:%lld) - no third line after: %s", fname, fpos, buf);
         }
         if (buf[0] != '+') {
             long long fpos = fqr->fpos;
             DIE("Improper fastq file (%s:%lld) - third line does not start with '+': %s", fname, fpos, buf);
         }
         if (!get_next_line(fqr)) {
             long long fpos = fqr->fpos;
             DIE("Improper fastq file (%s:%lld) - no fourth line after: %s", fname, fpos, buf);
         }
